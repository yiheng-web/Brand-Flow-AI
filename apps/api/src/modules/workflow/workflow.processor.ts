  ...
  async process(job: Job<RunWorkflowJobData>): Promise<AgentStateType | void> {
    ...
    try {
      ...
      const initialState: Partial<AgentStateType> = {
        ...
        knowledgeContext: knowledgeNode?.output?.knowledgeContext ? (knowledgeNode.output.knowledgeContext as any) : undefined,
        ...
        evaluationResult: evaluateNode?.output?.overallScore ? (evaluateNode.output as any) : undefined,
        retryCount: 0,
        status: "running",
      };
      ...
      for await (const chunk of stream) {
        ...
        if (nodeType && nodeType !== '__end__' && !skipNodes.includes(nodeType)) {
          ...
          else if (nodeType === 'knowledgeNode' && update.knowledgeContext) outputData = { knowledgeContext: update.knowledgeContext };
          else if (nodeType === 'promptNode' && update.promptResult) outputData = update.promptResult;
          else if (nodeType === 'generateNode' && update.generateResult) outputData = update.generateResult;
          else if (nodeType === 'evaluateNode' && update.evaluationResult) outputData = update.evaluationResult;
          ...
          await this.workflowNodeModel.findOneAndUpdate(
            ...
            {
              $set: {
                status: update.status === 'success' ? 'completed' : update.status === 'failed' ? 'failed' : 'running',
                output: outputData,
              }
            }
          );
