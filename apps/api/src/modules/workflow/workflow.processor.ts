...
export class WorkflowProcessor extends WorkerHost {
  ...
  async process(job: Job<RunWorkflowJobData>): Promise<AgentStateType | void> {
    ...
    try {
      ...
      const callableKnowledgeBaseIds =
        job.data.callableKnowledgeBaseIds ??
        (workflow.callableKnowledgeBaseIds ?? []).map((id: any) => id.toString())
      const selectedKnowledgeBaseIds =
        job.data.selectedKnowledgeBaseIds ??
        (workflow.selectedKnowledgeBaseIds ?? []).map((id: any) => id.toString())
      const requiredKnowledgeBaseIds =
        job.data.requiredKnowledgeBaseIds ??
        (workflow.requiredKnowledgeBaseIds ?? []).map((id: any) => id.toString())

      const initialState: Partial<AgentStateType> = {
        userQuery: workflow.prompt,
        context: {
          spaceId: job.data.spaceId ?? workflow.spaceId,
          spaceType: job.data.spaceType ?? workflow.spaceType,
          ownerUserId: job.data.ownerUserId ?? workflow.ownerUserId?.toString(),
          teamId: job.data.teamId ?? workflow.teamId?.toString(),
          enterpriseId: job.data.enterpriseId ?? workflow.enterpriseId?.toString(),
          knowledgeId: job.data.knowledgeId ?? callableKnowledgeBaseIds[0],
          selectedKnowledgeBaseIds,
          requiredKnowledgeBaseIds,
          callableKnowledgeBaseIds,
          brandRules: job.data.brandRules ?? workflow.brandRulesSnapshot,
          policies: job.data.policies ?? workflow.policiesSnapshot,
